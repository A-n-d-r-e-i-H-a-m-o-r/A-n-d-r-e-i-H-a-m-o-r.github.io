...
// document.addEventListener("click", someFunction); This is a DOCUMENT OBJECT
const left = document.getElementById("left-side"); //This is an ELEMENT OBJECT
const extra = document.getElementById("extra");

const handleOnMove = e /* "e" is like saying write the keyword "event" as e */ => {
    const p = e.clientX / window.innerWidth * 100;

    left.style.width = `${p}%`;
    
}
extra.onmousemove = e => handleOnMove(e);
...
function updateClock() {
    ...
    document.getElementById('digital').innerHTML = 
        `${hours}:${minutes} <span class="period">${period}</span>`;
}
...
//   	TO ADD A MOUSE CLICK OR ANY EVENT
// hi.addEventListener("click", showCoords); or
// hi.onclick = function(){showCoords(event)} or
// hi.onclick = write_whatever => showCoords(write_whatever);
...
//==================NOTE: THIS IS THE KEYWORD FOR addEventListener()========================
// ==================TO USE THIS SOMEWHERE ELSE ADD ON==========================
...
//=================THIS IS WHERE THE EVENT KEYWORD IS USED===========================
